...
fn dgreen_dn_src(obs: R3, src: R3, n_src: R3) -> Scalar {
    ...
    if r <= 1.0e-12 {
        ...
        -(r_vec.dot(&n_src)) / (4.0 * std::f64::consts::PI * r * r * r)
    }
}
    ...
    for i in 0..n {
        for j in 0..n {
            if i == j {
                ...
                d[(i, j)] = panels[j].area
                    * dgreen_dn_src(panels[i].centroid, panels[j].centroid, panels[j].normal);
            }
        }
    }
    d
...
#[must_use]
pub fn potential_from_single_layer(
    points: &[R3],
    sigma: &DVector<Scalar>,
    panels: &[FlatPanel],
) -> Vec<Scalar> {
...
#[must_use]
pub fn electric_field_from_single_layer(
    points: &[R3],
    sigma: &DVector<Scalar>,
    panels: &[FlatPanel],
) -> Vec<R3> {
...
#[must_use]
pub fn solve_dirichlet_single_layer(
    panels: &[FlatPanel],
    boundary_potential: &DVector<Scalar>,
...
mod tests {
    ...
    fn potential_far_field_scales_as_total_charge_over_r() {
        ...
        assert_relative_eq!(
            phi * 4.0 * std::f64::consts::PI * 100.0,
            1.0,
            max_relative = 5e-2
        );
    }
